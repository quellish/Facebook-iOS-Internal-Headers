//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Aug 12 2015 14:15:46).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2014 by Steve Nygard.
//

#import "CKCompositeComponent.h"

@class FBSearchComponentModel, FBSearchResultsContext;

@interface FBGraphSearchResultsBigFadingOutPhotoPandoraComponent : CKCompositeComponent
{
    FBSearchComponentModel *_moduleEdgeModel;
    FBSearchResultsContext *_context;
}

+ (id)newWithModuleEdgeModel:(id)arg1 context:(id)arg2 style:(const struct FBGraphSearchResultsBigFadingOutPhotoPandoraComponentStyle *)arg3;
@property(readonly, nonatomic) FBSearchResultsContext *context; // @synthesize context=_context;
@property(readonly, nonatomic) FBSearchComponentModel *moduleEdgeModel; // @synthesize moduleEdgeModel=_moduleEdgeModel;
- (void).cxx_destruct;
- (void)didTapSeeAll:(id)arg1;
- (void)didTapSeeAll:(id)arg1 withModels:(id)arg2;
- (void)didTapSeeMore:(id)arg1 withModels:(id)arg2;

@end

