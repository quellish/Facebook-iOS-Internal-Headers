//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Aug 12 2015 14:15:46).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2014 by Steve Nygard.
//

#import "FBReactionBaseActionAdapter.h"

@class FBReactionSessionInfo, FBUserSession, UIControl;

@interface FBReactionPhotosByCategoryActionAdapter : FBReactionBaseActionAdapter
{
    UIControl *_actionView;
    FBUserSession *_session;
    FBReactionSessionInfo *_reactionSessionInfo;
}

@property(readonly, nonatomic) FBReactionSessionInfo *reactionSessionInfo; // @synthesize reactionSessionInfo=_reactionSessionInfo;
@property(readonly, nonatomic) FBUserSession *session; // @synthesize session=_session;
- (void).cxx_destruct;
- (BOOL)performMagicTap;
- (void)_openCategorizedPhotoViewer;
- (void)performActionWithExtras:(id)arg1;
- (void)_actionViewTapped;
- (id)actionView;
- (id)initWithReactionContext:(id)arg1 resultStyle:(id)arg2 reactionUnit:(id)arg3 action:(id)arg4;

@end

