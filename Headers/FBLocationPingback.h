//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Aug 12 2015 14:15:46).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2014 by Steve Nygard.
//

#import <Foundation/NSObject.h>

#import "FBAppService-Protocol.h"

@class FBDelayServicesOnColdStartExperimentContext, FBIdleDetector, FBNetworkerRequest, FBUserSession, NSSet, NSString;
@protocol FBLocationManagerSubscription;

@interface FBLocationPingback : NSObject <FBAppService>
{
    FBUserSession *_session;
    unsigned int _state;
    FBNetworkerRequest *_networkRequest;
    id <FBLocationManagerSubscription> _locationManagerToken;
    FBIdleDetector *_idleDetector;
    FBDelayServicesOnColdStartExperimentContext *_delayServiceOnColdStartExperimentContext;
}

- (void).cxx_destruct;
- (void)_idleDetectionDidFired;
- (void)_networkRequestDidComplete;
- (void)_sendNetworkRequestForLocation:(id)arg1;
- (void)_unregisterLocationManager;
- (void)_handleError:(id)arg1 location:(id)arg2;
- (void)_requestLocationIfNeeded;
- (void)dealloc;
- (void)stopAppServiceWithDependencies:(id)arg1;
- (void)startAppServiceWithDependencies:(id)arg1;
@property(readonly, nonatomic) unsigned int serviceStartupLevel;
@property(readonly, copy, nonatomic) NSSet *appServiceDependencyIDs;
@property(readonly, copy, nonatomic) NSString *appServiceID;
- (id)initWithSession:(id)arg1;
- (id)init;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned int hash;
@property(readonly) Class superclass;

@end

